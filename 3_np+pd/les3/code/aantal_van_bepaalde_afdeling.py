...
def aantal_in_afdeling(df, afdeling):
    return df[df["afdeling"] == afdeling].shape[0]
...
totaal_loon_hr = hr_personeel["loon"].sum()
print(f"Totale loon voor afdeling HR: {totaal_loon_hr}")

# Aantal medewerkers in IT
aantal_it = aantal_in_afdeling(personeel, "IT")
print(f"Aantal medewerkers in IT: {aantal_it}")
