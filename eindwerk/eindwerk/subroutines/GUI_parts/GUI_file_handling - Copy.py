...
def get_data_header_and_dialect(filepath):
    ...
    masterframe.combobox_dt['state'] = 'normal'
    masterframe.combobox_dt.set(columns[0])  # The visible entry in combobox
    masterframe.combobox_dt.configure(values=columns)
    masterframe.file_import_button.configure(state="normal")
    # combobox_dt.update_idletasks()
    masterframe.combobox_dt['state'] = 'readonly'  # avoids writing by user in combobox
    masterframe.sample_textbox.delete("1.0", "end")
    masterframe.sample_textbox.insert("1.0", header_info['sample'])
...
def start_read_header_file(file_path):
    ...
    masterframe.label_0_1_file.configure(text=f"File: {file_}")
    masterframe.label_0_1_status.configure(text=f"Reading header: wait", text_color='red')
    masterframe.label_0_1_file.update_idletasks()  # Update screen immediately
    get_data_header_and_dialect(file_path)
    masterframe.label_0_1_status.configure(text=f" ")
    # Remove info about previous file label_0_1_rows.configure(text=f"# rows: {data_info['orig_rows']}")
    masterframe.label_0_1_rows.configure(text=f"..")
    masterframe.label_0_1_columns.configure(text=f"..")
    masterframe.label_0_1_cleaned.configure(text=f" . ")
    masterframe.label_0_1_clnd_rows.configure(text=f"..")
    masterframe.label_0_1_clnd_columns.configure(text=f"..")

def read_file(): # file_path, dialect are stored in the module
    masterframe.label_0_1_status.configure(text=f"Reading file: wait", text_color='red')
    masterframe.label_0_1_status.update_idletasks()  # Update screen immediately
    datetime_column = masterframe.combobox_dt.get()
    print("Selected datetime_column: ", datetime_column)
    datetime_format = masterframe.combobox_dtfor.get()
    print("Selected datetime_format: ", datetime_format)
    data, data_info = rd_csv.read_file(datetime_column, datetime_format)
    print("Data: ", data)
    masterframe.label_0_1_rows.configure(text=f"# rows: {data_info['orig_rows']}")
    masterframe.label_0_1_columns.configure(text=f"# columns: {data_info['orig_columns']}")
    masterframe.label_0_1_cleaned.configure(text=f"Cleaned data:")
    masterframe.label_0_1_clnd_rows.configure(text=f"# rows: {data_info['cleaned_rows']}")
    masterframe.label_0_1_clnd_columns.configure(text=f"# columns: {data_info['cleaned_columns']}")
    if datetime_column != 'no datetime':
        timestep = calculate_avg_timestep(data)
        masterframe.label_0_1_step.configure(text=f"avg. step: {timestep}")
    else:
        masterframe.label_0_1_step.configure(text=f"--")
    masterframe.label_0_1_status.configure(text=f"  ")
...
def Place_frame1(masterframe, callback):
    ...
    file_import_button = ctk.CTkButton(masterframe, text="File history", image=open_folder_img, \
    ...
    masterframe.label_0_1_file = ctk.CTkLabel(masterframe, text=f" Filepath: (empty)")
    masterframe.label_0_1_file.grid(column=0, row=3, sticky="nw", padx=2, pady=2)

    # Create label to indicate status (read header, read file)
    masterframe.label_0_1_status = ctk.CTkLabel(masterframe, text=f" ")  # empty when no action happens
    masterframe.label_0_1_status.grid(column=0, row=4, sticky="nw", padx=2, pady=2)

    # Combobox to know datetime column
    combobox_label_dt = ctk.CTkLabel(masterframe, text="Choose datetime column")
    combobox_label_dt.grid(column=1, row=0, sticky="nw", padx=2, pady=2)
    combobox_dt_var = tk.StringVar(masterframe, "empty")  # The visible entry in combobox
    masterframe.combobox_dt = ctk.CTkComboBox(masterframe, values=["empty"])
    masterframe.combobox_dt['state'] = 'readonly'
    masterframe.combobox_dt.grid(column=1, row=1, sticky="nw", padx=2, pady=2)

    # Combobox to know datetime format
    combobox_label_dtfor = ctk.CTkLabel(masterframe, text="Select or write datetime format")
    combobox_label_dtfor.grid(column=1, row=2, sticky="nw", padx=5, pady=0)
    combobox_dtfor = ctk.CTkComboBox(masterframe, values=datetime_format_list)
    ...
    file_import_button = ctk.CTkButton(masterframe, text="Read file", image=open_folder_img, compound='left', \
    ...
    #  First row in frame
    label_0_1_smpl = ctk.CTkLabel(masterframe, text=f"Sample data fom the file")
    label_0_1_smpl.grid(column=2, row=0, sticky="nw", padx=2, pady=2)
    masterframe.label_0_1_rows = ctk.CTkLabel(masterframe,     text=f" .          .", width=100)
    masterframe.label_0_1_columns = ctk.CTkLabel(masterframe,  text=f" ..        ..", width=100)
    masterframe.label_0_1_rows.grid(column=3, row=0, sticky="nw", padx=2, pady=2)
    masterframe.label_0_1_columns.grid(column=4, row=0, sticky="nw", padx=2, pady=2)

    # Create textbox at the right with sample data and scrollbars
    masterframe.sample_textbox = ctk.CTkTextbox(masterframe, wrap="none", height=100)  # , state="disabled" means read-only , height=140
    masterframe.sample_textbox.grid(column=2, row=1, sticky="nw", columnspan=4, rowspan=3)  #
    # sample_textbox.grid_rowconfigure(0, weight=1)
    # sample_textbox.grid_columnconfigure(0, weight=1)
    # Create a Text widget
    masterframe.sample_textbox.insert("1.1", f"empty")
    #  Bottom row in frame
    masterframe.label_0_1_cleaned = ctk.CTkLabel(masterframe, text=f" . ")  # This serves to show cleaned data statistics later
    masterframe.label_0_1_cleaned.grid(column=2, row=4, sticky="nw", padx=2, pady=2)
    label_0_1_clnd_rows = ctk.CTkLabel(masterframe,     text=f" .          .", width=100)
    label_0_1_clnd_columns = ctk.CTkLabel(masterframe,  text=f" ..        ..", width=100)
    label_0_1_step = ctk.CTkLabel(masterframe,     text=f" ...      ...", width=100)
...
if __name__ == '__main__':
    ...
    masterframe = ctk.CTk()
    masterframe.title('Test Frame filehandling')
